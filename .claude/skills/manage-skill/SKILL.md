...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| verify-site-sync | index.html ↔ sidebar.js ↔ 실제 페이지 동기화 검증 및 자동 수정 | `index.html`, `sidebar.js`, `cheatsheets/*.html`, `links/*.html` |
| verify-cheatsheet-structure | 치트시트 HTML/CSS 구조 및 접근성 검증 | `cheatsheets/*.html`, `cheatsheets/*.css` |
